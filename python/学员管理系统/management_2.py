...
def vlan():
    ...
    format = [
        [gui.Text('学员账号管理', size=(90, 0), justification='center', font=(80), )],
    ...
    while True:
        ...
        if event == '删除指定账号':
            ...
            vlan1.start()
        if event == '添加指定账号':
            vlan2 = multiprocessing.Process(target=register.input)
            vlan2.start()
    ...
    while True:
        ...
        if event_delete == '确定':
            ...
            try:
                ...
                with open('data/user.txt', 'w', encoding='utf-8')as fpr:
                    fpr.write('')
                for it in users_passwords:
                    if it == '\n':
                        pass
                    else:
                        with open('data/user.txt','a',encoding='utf-8')as fprt:
                            fprt.write(it)
